...
# data: 6 recorded values + 2 failures
...
def log_p_fail(th, sig):
    p = Phi((a - th) / sig) + (1.0 - Phi((b - th) / sig))
    return -np.inf if p <= 0 else math.log(p)
...
for i, th in enumerate(theta):
    for j, ls in enumerate(logsig):
        ...
        lpF = log_p_fail(th, sig)
        if not np.isfinite(lpF):
            continue
        logW[i, j] = (-n * ls) + s_logphi + (m_fail * lpF) # posterior on (theta, log_sigma) up to const
